...
(function (LabelPlus) {
    ...
    function closeImage(img, saveType) {
        ...
        if (img.ws.bgLayer) {
            try {
                if (img.ws.doc.layers[img.ws.doc.layers.length - 1] !== img.ws.bgLayer) {
                    img.ws.bgLayer.move(img.ws.doc.layers[img.ws.doc.layers.length - 1], ElementPlacement.PLACEAFTER);
                }
                img.ws.doc.activeLayer = img.ws.bgLayer;
                var desc = new ActionDescriptor();
                var ref = new ActionReference();
                ref.putClass(cTID("BckL"));
                desc.putReference(cTID("null"), ref);
                var ref2 = new ActionReference();
                ref2.putEnumerated(cTID("Lyr "), cTID("Lyr "), cTID("Trgt"));
                desc.putReference(cTID("Usng"), ref2);
                executeAction(cTID("Mk  "), desc, DialogModes.NO);
                LabelPlus.log('set bg layer as background layer');
            } catch (e) {
                LabelPlus.log('set bg layer as background layer failed: ' + e);
            }
        }
